...
const infor = new Router();
//处理get请求
infor.post("/add", async ctx => {
     //从前端获取过来的数据
     let data = ctx.request.body.fields;
     //将合的数据存入到数据库中
     let result = await db.insertData('infor', data);
     //返回操作的结果
     console.log(123456);
  ...
  .get("/del", async ctx => {
    let data = ctx.query;
    let result = await db.deleteDataById('infor', data.id);
    ctx.body = result.affectedRows ? errInfo.DEL_SUCCESS : errInfo.DEL_FAIL;
  })
  .post("/update", async ctx => {
    //处理更新请求
    let data = ctx.request.body.fields;
    let result = await db.updatetById('infor', data, data.id);
  ...
  .get("/select",async (ctx)=>{
    ...
    delete data.pn;
    //获取总条数
    let count = (await db.count("infor", data))[0].total_count;
    let json = {
        table: "infor",
        where: data,
        start: start,
