...
comment.post("/add", async ctx => {
     ...
     //将合的数据存入到数据库中
     let result = await db.insertData('comment', data);
     //返回操作的结果
     ctx.body = result.affectedRows ? errInfo.ADD_SUCCESS : errInfo.ADD_FAIL;
  })
  .get("/del", async ctx => {
    let data = ctx.query;
    let result = await db.deleteDataById('comment', data.id);
    ctx.body = result.affectedRows ? errInfo.DEL_SUCCESS : errInfo.DEL_FAIL;
  })
  .post("/update", async ctx => {
    //处理更新请求
    let data = ctx.request.body.fields;
    let result = await db.updatetById('comment', data, data.id);
  ...
  .get("/select",async (ctx)=>{
    ...
    delete data.pn;
    //获取总条数
    let count = (await db.count("comment", data))[0].total_count;
    let json = {
        table: "comment",
        where: data,
        start: start,
